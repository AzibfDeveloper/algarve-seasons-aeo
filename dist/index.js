...
async function getUserFiles(userId, category, limit = 20, offset = 0) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  const whereClause = category ? and(eq(files.userId, userId), eq(files.category, category)) : eq(files.userId, userId);
  const result = await db.select().from(files).where(whereClause).orderBy(desc(files.createdAt)).limit(limit).offset(offset);
  return result;
}
...
async function getFileCount(userId, category) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  const whereClause = category ? and(eq(files.userId, userId), eq(files.category, category)) : eq(files.userId, userId);
  const result = await db.select().from(files).where(whereClause);
...
var appRouter = router({
  ...
  files: router({
    upload: protectedProcedure.input(z2.object({
      ...
      mimeType: z2.string().default("application/octet-stream"),
      category: z2.string().max(100).default("general")
      ...
      try {
        ...
        const fileKey = `${ctx.user.id}/files/${input.category}/${timestamp2}-${input.fileName}`;
    ...
    getDownloadUrl: protectedProcedure.input(z2.object({ fileId: z2.number() })).query(async ({ ctx, input }) => {
      try {
